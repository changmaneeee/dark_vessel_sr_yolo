        ...
        if model.use_selective_inference:
            ...
            if sr_selected_mask.any():
                ...
                if model.blend_selected_inference:
                    selected_gate = gate_flat[sr_selected_mask].view(-1, 1, 1, 1).to(dtype=hr_dtype)
                    bypass_sel = upsampled[sr_selected_mask]
                    selected_hr = selected_gate * selected_sr + (1.0 - selected_gate) * bypass_sel
                else:
            ...
            # full blend 경로도 dtype 맞춤
            if sr_image.dtype != upsampled.dtype:
                sr_image = sr_image.to(dtype=upsampled.dtype)

            gate_expanded = gate_flat.view(batch, 1, 1, 1).to(dtype=upsampled.dtype)
...
def build_output(
    ...
    ratio_gt = float((gate_arr > 0.5).mean()) if gate_arr.size else None
